...
OUTPUT_FILE = "final_hierarchical_output_v9.jsonl" # <-- V9
MAX_NEW_TOKENS = 2048
...
# --- 5. Master Prompt Templates (LUNA'S V9 CHAT-BASED FIX) ---
...
**INSTRUCTIONS:**
1.  ★★★ **TARGET SELECTION:** The provided dialogue uses 'Client:' and 'Counselor:' prefixes. **You MUST ONLY extract and analyze the utterances that begin with 'Client:'.** You MUST ignore all 'Counselor:' lines.
2.  ★★★ **UTTERANCE INTEGRITY:** Each block of text starting with 'Client:' is a *single, complete utterance*, even if it contains multiple sentences or paragraphs. **Do NOT split a single 'Client:' block into multiple `directed_utterances` entries.**
3.  **ANALYSIS:** For each 'Client:' utterance you extract, perform a two-level analysis:
...
4.  ★★★ **MAINTAIN EMOTIONAL COHERENCE:** The emotional direction of an utterance MUST be a logical progression from the *previous client utterance*. Abrupt, unrealistic emotional shifts are forbidden.
...
source_data_list = []
chats_list = [] # <-- V8: Changed from prompts_list
try:
    with open(INPUT_FILE, 'r', encoding='utf-8') as infile:
        for line in infile:
            ...
            if dialogue:
                # <-- V8: Create chat message structure
                chat_messages = [
...
remaining_chats = chats_list[processed_count:] # <-- V8
...
with open(OUTPUT_FILE, 'a', encoding='utf-8') as outfile:
    if not remaining_chats:
        ...
        for i, chat in enumerate(tqdm(remaining_chats, desc="Processing Dialogues")): # <-- V8
