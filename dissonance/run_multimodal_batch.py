#!/usr/bin/env python3
"""
Batch run Multimodal (Vocal-Aware) therapist generation for N dialogues.
Extracted from dialogue_1_multimodal.ipynb.
Same pipeline as Dissonance but WITHOUT delta computation, is_dissonant flag,
or emotion_mismatch — to isolate the effect of the dissonance signal.
"""
import argparse
import os
import sys
...
# =====================================================
# Client prompts (identical to all conditions)
# =====================================================
CLIENT_SYSTEM = """
...
# =====================================================
# Therapist prompts (Multimodal Vocal-Aware — NO dissonance)
# =====================================================
THERAPIST_SYSTEM = """
...
def run_single_dialogue(dialogue_id: int, max_turns: int = 10):
    ...
    print(f"\n=== MULTIMODAL (VOCAL-AWARE) dialogue {dialogue_id} ===")
    ...
    print(f"  VA: text=({val_t:.2f},{aro_t:.2f}) speech=({val_s:.2f},{aro_s:.2f}) | emo={text_emo}/{speech_emo} | vocal={vocal_desc[:40]}...")
    ...
    base_name = f"multimodal_{dialogue_id}_full_multimodal"
