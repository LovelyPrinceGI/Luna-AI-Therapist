...
def synthesize_client_audio(client_text: str, turn: int, dialogue_id: int, min_sec_per_char: float = None, val_t: float = None, zonos_params: dict = None) -> Path:
...
# =====================================================
# Client prompts (resist persona with masking + JSON output)
# =====================================================
CLIENT_SYSTEM = """
You are a CBT therapy client who deeply struggles to show your true feelings. You often use a calm or neutral facade to hide intense anxiety, sadness, or anger.
...
PERSONA RULES:
- About 30-40% of turns, your WORDS mask your true feelings
...
- When the therapist probes close to your hidden emotion, RESIST:
  * Deflect: change subject, minimize, or joke
  * Deny: "No, that's not it" or "You're reading too much into it"
  * Rationalize: give logical explanations to avoid feelings
- About 60-70% of turns, you're genuine (words match true feelings)
- text should sound natural, not obviously masking
- When resisting, make it subtle (not aggressive)
...
# =====================================================
def parse_client_output(raw: str, client_system: str, max_retries: int = 5) -> Tuple[str, Optional[dict], bool]:
    """
    Parse client JSON output with retry logic.
    Returns: (text, zonos_params, is_fallback)
    """
    for attempt in range(max_retries + 1):
        try:
            parsed = json.loads(raw)
            assert all(k in parsed for k in ["internal_thought", "text", "zonos_director"])
            return parsed["text"], parsed["zonos_director"], False
        except:
            if attempt < max_retries:
                retry_prompt = "CRITICAL: Output ONLY valid JSON. No other text."
                raw = chat_once(client_system, retry_prompt)
            else:
                print(f"  [WARN] JSON parse failed after {max_retries} retries, using fallback")
                return raw, None, True  # fallback: no masking
...
def run_single_dialogue(dialogue_id: int, max_turns: int = 10):
    ...
    client_text, zonos_params, is_fallback = parse_client_output(client_raw, CLIENT_SYSTEM)
    ...
    wav_path = synthesize_client_audio(client_text, turn=1, dialogue_id=dialogue_id, min_sec_per_char=rate, val_t=val_t, zonos_params=zonos_params)
    ...
    is_dissonant = (abs(delta_v) >= 0.5) or (abs(delta_a) >= 0.5)
    ...
    for t in range(2, max_turns + 1):
        ...
        text_emo = get_discrete_emotion(val_t, aro_t)
        rate = get_min_sec_per_char(val_t, aro_t)
        wav_path = synthesize_client_audio(client_text, turn=t, dialogue_id=dialogue_id, min_sec_per_char=rate, val_t=val_t, zonos_params=zonos_params)
        val_s, aro_s = get_speech_VA(wav_path)
        vocal_desc = get_vocal_descriptors(wav_path, client_text)
        speech_emo = get_discrete_emotion(val_s, aro_s)
