...
DIALOGUE_DIR = os.path.join(BASE_DIR, "dissonance", "own_script", "asmr")

# ใช้ ZONOS_INPUT_JSON ถ้ามี; ถ้าไม่มี fallback ไป dialogue_6_directed_zonos.json
DEFAULT_INPUT_JSON = os.path.join(DIALOGUE_DIR, f"asmr_{DIALOGUE_ID}_directed_zonos.json")
...
MAX_RETRIES = 2      # เดิม 2
MIN_SECONDS_PER_CHAR = 0.05 # เพิ่มให้คาดว่าเสียงยาวขึ้นนิดนึง
...
# เพิ่ม prefix="client" เข้ามาเป็นค่าเริ่มต้นค่ะ
def synth_single_utterance(idx: int, input_json: str, prefix: str = "client"):
    """
    idx = dialogue turn (1,2,3,...)
    input_json = path ไป JSON ที่มี directed_utterances (ของ turn นี้)
    prefix = ระบุว่าเป็น "client" หรือ "sister" เพื่อแยกเสียงและชื่อไฟล์
    """
    print(f"[{prefix.upper()}] Using INPUT_JSON:", input_json)
    ...
    # ----------------------------------------------------
    # จุดแก้ที่ 1: เลือก Speaker ตามตัวละครค่ะ
    # ----------------------------------------------------
    if prefix == "sister":
        selected_speaker_tensor = female_emb  # เสียงพี่สาวลูน่า
    else:
        selected_speaker_tensor = male_emb    # เสียงไดจิ (Client)
    # ----------------------------------------------------
    ...
    print(f"Expected minimum duration ~{expected_min_sec:.2f}s for utterance {idx}")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(f"[Zonos - {prefix}] Utterance {idx} attempt {attempt}/{MAX_RETRIES}")
            ...
            if y_trimmed.size == 0:
                print("Only silence after trim at utterance", idx, "attempt", attempt)
                continue
            ...
            if duration_sec >= expected_min_sec and rms >= MIN_RMS:
                ...
                # ----------------------------------------------------
                # จุดแก้ที่ 2: ใส่ prefix ลงไปในชื่อไฟล์ จะได้ไม่ทับกัน
                # ----------------------------------------------------
                out_name = f"{prefix}_dialogue_{DIALOGUE_ID}_utterance_{idx}.wav"
                out_path = os.path.join(OUTPUT_AUDIO_DIR, out_name)
                write_wav(out_path, model_sr, audio_out)
                print(f"Saved: {out_path} (dur={duration_sec:.2f}s, rms={rms:.3f}, attempt {attempt})")
                return out_path
    ...
    if best_out is not None:
        ...
        # ----------------------------------------------------
        # แก้ชื่อไฟล์ตรง Fallback ด้วยค่ะ
        # ----------------------------------------------------
        out_name = f"{prefix}_dialogue_{DIALOGUE_ID}_utterance_{idx}.wav"
        out_path = os.path.join(OUTPUT_AUDIO_DIR, out_name)
        write_wav(out_path, sr, audio_out)
        print(f"[FALLBACK] Saved best-effort audio for utterance {idx} (dur={best_dur:.2f}s, rms={best_rms:.3f})")
        return out_path
