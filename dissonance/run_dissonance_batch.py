#!/usr/bin/env python3
"""
Batch run Dissonance-Aware therapist generation for N dialogues.
Extracted from dialogue_1_dissonance_resist.ipynb.
Supports incremental runs via --start/--end arguments.
Runs on WSL2 with Flash Attention for 2-3x Zonos speedup.
"""
import argparse
import os
import sys
...
def get_speech_VA(wav_path: Path) -> Tuple[float, float]:
    ...
    return _scale_0_1_to_minus1_1(val), _scale_0_1_to_minus1_1(aro)
...
# =====================================================
# Zonos Director LLM (vocal parameter assignment)
# =====================================================
...
def synthesize_client_audio(client_text: str, turn: int, dialogue_id: int, min_sec_per_char: float = None, val_t: float = None) -> Path:
    ...
    out_path_str = synth_single_utterance(turn, str(TMP_ZONOS_JSON), dialogue_id=dialogue_id, prefix="dissonance", min_sec_per_char=min_sec_per_char)
    if out_path_str is None:
        raise RuntimeError(f"Zonos failed to synthesize utterance {turn}")
...
# Dynamic speaking rate: higher for sad/depressed (slow), lower for excited (fast)
def get_min_sec_per_char(val_t: float, aro_t: float) -> float:
    base = 0.035
    va_factor = val_t * -0.002 + aro_t * -0.007  # arousal drives speech rate more than valence
...
def get_problem_seed(dialogue_id: int) -> str:
    return SHARED_SEEDS[dialogue_id - 1]
...
# =====================================================
# Main loop: run a single dialogue
# =====================================================
def run_single_dialogue(dialogue_id: int, max_turns: int = 10):
    problem_seed = get_problem_seed(dialogue_id)
    print(f"\n=== DISSONANCE-AWARE dialogue {dialogue_id} ===")
    ...
    print(f"  VA: text=({val_t:.2f},{aro_t:.2f}) speech=({val_s:.2f},{aro_s:.2f}) | delta=({delta_v:.2f},{delta_a:.2f}) | dissonance={is_dissonant} | emo={text_emo}/{speech_emo} mismatch={emotion_mismatch}")

    therapist_text = chat_once(
        THERAPIST_SYSTEM,
        THERAPIST_USER_TEMPLATE.format(
...
if __name__ == "__main__":
    ...
    print(f"Dissonance-Aware batch: dialogues {args.start} to {args.end}")
